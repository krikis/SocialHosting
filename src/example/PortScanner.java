...
public class PortScanner {
	/**
	 * Process arguments. Make a PortFactory. Make and start a bunch of
	 * ScannerThreads.
	 * 
	...
	public static void main(String[] argv) {
		...
		while (nextArg < argv.length) {
			...
			try {
				if (arg.equalsIgnoreCase("-threads")) {
					...
					System.out
							.println("arguments:  -host s [-threads n] [-ports n m]");
					System.exit(0);
				} else {
				...
				badArg("bad number format for " + arg + ": "
						+ argv[nextArg - 1]);
		...
		System.out.println("host: " + host + " ports: " + firstPort + "-"
				+ lastPort + " threads: " + threads);
